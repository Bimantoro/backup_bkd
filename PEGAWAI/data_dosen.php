...
class Data_dosen extends CI_Controller {
	...
	function _status_DS_serdos($kd_dosen, $kd_ta, $kd_smt){
	...
	function _status_DS($kd_dosen, $kd_ta, $kd_smt){
		...
		$fungs = $this->_cek_jab_fungsional($kd_dosen, $kd_ta, $kd_smt);
		if($struk == 2){
			$status = 'DK'; //Dosen dengan Tugas Khusus
		}else if ($struk == 1){
	...
	function testing_status_ds($nip){
		echo $this->_status_DS($nip, '2017', '2');
	}
	...
	function cek_api(){
		...
		$data = $this->s00_lib_api->get_api_json(
			$url,
			'POST',
			array('api_kode'=>19000, 'api_subkode'=>3, 'api_search' => array())
		);
	...
	function testing_tanggal(){
		$semester = $this->s00_lib_api->get_api_json(URL_API_SIA.'sia_krs/data_search', 'POST', array('api_kode'=>52000, 'api_subkode'=>12, 'api_search' => array('10-12-2017', '0')));
		...
		$bulan_awal = substr($semester[0]['TGL_MULAI_SMT'], 3,2);
		$bulan_akhir = substr($semester[0]['TGL_AKHIR_SMT'], 3,2);

		$tahun_awal = substr($semester[0]['TGL_MULAI_SMT'], -4);
		$tahun_akhir = substr($semester[0]['TGL_AKHIR_SMT'], -4);
		
		$data = array();
		if($tahun_awal == $tahun_akhir){
			for ($i=(int) $bulan_awal; $i <= (int) $bulan_akhir; $i++) { 
				$temp = array('TAHUN' => $tahun_awal, 'BULAN' => $bulan[$i]);
				$data[] = $temp;
			}
		}else{
			//tahun pertama :
			for ($i=(int) $bulan_awal; $i <= (int) $akhir_bulan; $i++) { 
				$temp = array('TAHUN' => $tahun_awal, 'BULAN' => $bulan[$i]);
				$data[] = $temp;
			}

			for ($i=(int) $awal_bulan; $i <= (int) $bulan_akhir; $i++) { 
				$temp = array('TAHUN' => $tahun_akhir, 'BULAN' => $bulan[$i]);
				$data[] = $temp;
			}
	...
	function get_presensi_periode($kd_dosen='197701032005011003', $kd_periode = ''){
		$temp_status_presensi = 1;
		$total_hadir = 0;
		$total_kerja = 0;

		$rg_periode = $this->s00_lib_api->get_api_json(URL_API_SIA.'sia_krs/data_search', 'POST', array('api_kode'=>52000, 'api_subkode'=>12, 'api_search' => array('10-01-2018', '0')));
		//yang diatas cuman sample
		$bulan = array(1 => '01', '02', '03', '04', '05', '06', '07', '08', '09', '10', '11', '12');
		$awal_bulan = 1;
		$akhir_bulan = 12;

		$bulan_awal = substr($rg_periode[0]['TGL_MULAI_SMT'], 3,2);
		$bulan_akhir = substr($rg_periode[0]['TGL_AKHIR_SMT'], 3,2);

		$tahun_awal = substr($rg_periode[0]['TGL_MULAI_SMT'], -4);
		$tahun_akhir = substr($rg_periode[0]['TGL_AKHIR_SMT'], -4);
		...
		foreach ($data as $d) {
			$url 		= 'http://service5.uin-suka.ac.id/servremun/remun/api/';
			$parameter 	= array('ID_PEGAWAI'=> $kd_dosen, 'TAHUN'=>$d['TAHUN'], 'BULAN'=>$d['BULAN']);
			$dt 		= $this->s00_lib_api->get_api_json($url.'kehadiran_pegawai', 'POST', $parameter);

			if(!empty($dt)){
				$total_kerja += $dt[0]['JML_HARI'];
				$total_hadir += $dt[0]['JML_HADIR'];
			}else{
				$temp_status_presensi = 0;
			}
		}

		if($temp_status_presensi == 1){
			$presensi = number_format(($total_hadir / $total_kerja * 100), 2); 	
		}else{
			$presensi = "BELUM DI UPLOAD";
		}
