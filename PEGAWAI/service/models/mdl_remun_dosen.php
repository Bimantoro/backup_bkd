...
class Mdl_remun_dosen extends CI_Model {
	...
	//TAMBAHAN BASED ON PERIODE
	function get_current_aturan_agregasi($ta, $per){
		$sql = "SELECT * FROM remun_aturan_agregasi WHERE kd_ta = '$ta' AND kd_smt = '$per' AND status = 1";
		$q   = $this->db_remun->query($sql);
		return $q->result_array();
	}

	function get_current_aturan_agregasi_dsn($status, $ta, $per){
		$sql = "SELECT * FROM remun_aturan_agregasi WHERE status_dosen = '$status' AND kd_ta = '$ta' AND kd_smt = '$per' AND status = 1";
		$q   = $this->db_remun->query($sql);
		return $q->result_array();
	}

	function get_current_nilai_max_agregasi($ta, $per){
		$sql = "SELECT * FROM nilai_max_komponen_agregasi WHERE kd_ta = '$ta' AND kd_smt = '$per' AND status = 1";
		$q   = $this->db_remun->query($sql);
		return $q->result_array();
	}

	function get_current_aturan_agregasi_ikd($ta, $per){
		$query = "SELECT * FROM aturan_hitung_agregasi_ikd WHERE kd_ta = '$ta' AND kd_smt = '$per' ORDER BY id_aturan DESC";
		$sql = $this->db_remun->query($query);
		return $sql->result_array();
	}
