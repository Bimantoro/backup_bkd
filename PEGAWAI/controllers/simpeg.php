...
class Simpeg extends CI_Controller {
  ...
  function cek_jabatan($nip){
	...
	function bawahan_($nip){
		set_time_limit(300);
		//$nn = $this->bawahan($nip);
		//echo $kd_jabatan_pegawai . '<br>';
		//return $this->simpeg_jabatan_struktural($kd_pegawai);
		//$kd_jabatan_pegawai = $this->jabatan($nip)['DETAIL']['STR_ID'];
		//$data = $this->simpeg_ranting_pegawai('bawah', $nip)[$kd_jabatan_pegawai];
		//$tanggal = $tanggal ? $tanggal : date('d-m-Y');
		//$data = $this->api->post($this->url->simpeg_search, array('api_kode' => 1215, 'api_subkode' => 1, 'api_search' => array($tanggal, $nip, 1, 1)));
		//$data = $this->simpeg_ranting_pegawai('bawah', $nip, date('d-m-Y'));
