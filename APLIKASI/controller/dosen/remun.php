...
if (!defined ('BASEPATH')) exit ('Direct access not allowed...');
 ...
 class Remun extends CI_Controller{
	...
	function data($jenis, $detail = null, $action = null){
		...
		$kd_detil = $this->security->xss_clean($this->uri->segment(6));
		$data['kode'] = $jenis;
		...
		foreach ($data['data_remun'] as $key) {
			...
			//TAMBAHAN UNTUK MENAMPILKAN BATAS :
			if(isset($key->KETERANGAN)){
				$min_mhs = $this->_cek_batas_minimal_mhs($key->KETERANGAN);
				//echo "minimal jumlah mahasiswa untuk ".$key->JENIS_KEGIATAN." adalah : ".$min_mhs."<br>";
...
/*
	...
	function get_data_mhs_test($nim=1320012040)
		...
		function kesimpulan(){
			...
			$kd_smt = $this->_generate_smt($data['smt']);
			$this->session->unset_userdata('jenis_dosen');
			$this->session->set_userdata('jenis_dosen', $this->history->_status_DS_remun($data['kd_dosen'], $data['ta'], $data['smt']));
	...
	function t1(){
			$status =  $this->history->_status_DS_remun('198505142015031002', '2017', '2');
			$nama = $this->setting->_jenisDosen($status);
