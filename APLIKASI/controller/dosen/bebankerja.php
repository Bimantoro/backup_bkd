...
class Bebankerja extends CI_Controller{
	...
	*/
	...
	function profesor($param = '', $show = FALSE, $action = ''){
		...
		if ($this->session->userdata("jenis_dosen") == "PR" || $this->session->userdata("jenis_dosen") == "PT" || $kd=='197701032005011003'){
			// data beban
			...
			if($a == 'A'){
				...
				if ($a == 'B'){
					...
					if($simpan){
						...
						$parameter	= array('api_search' => array($kd_bk, $bt_mulai, $bt_selesai, $judul_penelitian, $sumber_dana, $jumlah_dana, $kd_kat, $jenjang, $tempat, $jml_mhs, $jml_sks, $jml_tatap_muka, $jenis_kelas, $pertemuan_pm, $jml_dosen,  $nm_prodi, $keterangan, $status_pindah));
						$this->s00_lib_api->get_api_jsob($api_url,'POST',$parameter);
						...
						$this->session->set_flashdata('msg', array('danger', 'Data gagal disimpan di tabel pengabdian'));
					}
					
				}else if($a=='D'){
					#SIMPAN PENUNJANG
					if($kd_kat == 71 || $kd_kat == 132){
						$jml_mhs = $this->input->post('jml_mhs');
					}else{
						$jml_mhs = 1;
					}
					...
					/*$jml_mhs = $this->input->post('jml_mhs');*/
					if($simpan){
						$api_url 	= URL_API_BKD.'bkd_beban_kerja/simpan_data_penunjang';
						$parameter	= array('api_search' => array($kd_bk, $kd_kat, $judul_penelitian, $jenjang, $tempat, $jml_mhs, $sks_rule_1, $jml_tatap_muka, $jenis_kelas, $pertemuan_pm, $jml_dosen,  $nm_prodi, $keterangan, $status_pindah));
						$save = $this->s00_lib_api->get_api_jsob($api_url,'POST',$parameter);
	...
	*/
	...
	function data_penawaran_kelas(){
				...
				foreach ($data['penawaran'] as $data){
					...
					/*if($status_verifikasi == 1){*/
						...
						if($jn_mk == 'UMUM'){
							...
							$nm_keg = $jenis_kegiatan; 
							$nm_keg = str_replace("'", "", $nm_keg);
							$nm_keg = strip_tags($nm_keg);

							//"Mengajar Matakuliah ".$data->NM_MK;
							...
							$nm_prodi = str_replace("'", "", $nm_prodi);
							$nm_prodi = strip_tags($nm_prodi);
							...
							if(!$cek_data_pengajaran){
								...
								$jalur_data = 0;
								if($status_pindah == 0){
									$jalur_data = $this->cek_jalur_data_kat(1);
								}
								...
								$api_url 	= URL_API_BKD.'bkd_beban_kerja/simpan_beban_kerja_sia_maps';
								$parameter  = array('api_search' => array($kd_dosen, 'A', $nm_keg, $bukti_penugasan, $sks_rule, $masa_penugasan, $sks_rule, $bkt_dokumen, $thn, $semester, 'LANJUTKAN','100', $ta, $smt, $sts_pakai));
								$simpan = $this->s00_lib_api->get_api_jsob($api_url,'POST',$parameter);
								if ($simpan){
									# get last id beban kerja
		...
		$flash_remun = $this->session->flashdata('remunerasi');
		if($flash_remun){
			$this->session->set_flashdata('bebankerja', true);
	...
	function get_data_mhs($nim)
		...
		function jml_mhs_ujian_semprop(){
			...
			if($data){
				foreach ($data as $key) {
					...
					$nilai_semprop = $this->get_status_semprop($nim);

					if($ta == $kd_ta_data && $kd_smt == $kd_smt_data && $nilai_semprop){
						$jumlah_mahasiswa++;
					}
				...
				foreach ($mhs as $key) {
					...
					if(!$cek_bimbingan){						
						...
						if($simpan){
							...
							$api_url 	= URL_API_BKD.'bkd_beban_kerja/simpan_data_penunjang_ta_maps';
		...
		function auto_bimbingan_dosen(){
		 	...
		 	if($kkn <= 0){
				...
				if($kd_bk){
					...
					if($simpan){
						...
						$api_url 	= URL_API_BKD.'bkd_beban_kerja/simpan_data_pendidikan_ta';
						$parameter	= array('api_search' => array($getid, $kd_kat, $nm_keg, 'S1', '-', $kkn, 1, '1', '1', 'A', '1', '-', '-', $status_pindah));
						$simpan = $this->s00_lib_api->get_api_jsob($api_url,'POST',$parameter);
					}
				}
		 	}
			...
			if($kp <= 0){
				...
				if($kd_bk){
					...
					if($simpan){
						...
						$api_url 	= URL_API_BKD.'bkd_beban_kerja/simpan_data_pendidikan_ta';
						$parameter	= array('api_search' => array($getid, $kd_kat, $nm_keg, 'S1', '-', $kp, 1, '1', '1', 'A', '1', '-', '-', $status_pindah));
						$simpan = $this->s00_lib_api->get_api_jsob($api_url,'POST',$parameter);
					}
				}				
			}
			...
			if($semprop){
				$kd_kat 	= 8; //ini untuk membimbing seminar proposal
		 		...
		 		if($kd_bk){
		 			$api_url 	= URL_API_BKD.'bkd_beban_kerja/update_jml_mhs_bimbingan';
					$parameter	= array('api_search' => array($kd_bk, $semprop));
					$update_bk	= $this->api->get_api_json($api_url, 'POST', $parameter);
		 		}else{
			...
			//MENGUJI SEMPROP SEKALIAN DISINI :

			$uji_semprop = $this->jml_mhs_ujian_semprop();
			if($uji_semprop > 0){
				$kd_kat 	= 113; //ini untuk membimbing seminar proposal
		 		$kd_jbk 	= 'A';
		 		$kd_dosen 	= $this->session->userdata('kd_dosen');
		 		$kd_ta 		= $this->session->userdata('kd_ta');
		 		$kd_smt 	= $this->session->userdata('kd_smt');
		 		...
		 		if($kd_bk){
		 			$api_url 	= URL_API_BKD.'bkd_beban_kerja/update_jml_mhs_bimbingan';
					$parameter	= array('api_search' => array($kd_bk, $uji_semprop));
					$update_bk	= $this->api->get_api_json($api_url, 'POST', $parameter);
		 		}else{

		 			$jml_sks = 0;
		 			$narasi = "Menguji seminar proposal";

		 			$jml_mhs = $uji_semprop;
		 			$sks_rule = $this->sksrule->_nilai_sks($jml_mhs, 1003, 1);
					...
					if($simpan){
						...
						$api_url 	= URL_API_BKD.'bkd_beban_kerja/simpan_data_pendidikan_ta';
						$parameter	= array('api_search' => array($getid, $kd_kat, $nm_keg, 'S1', '-', $jml_mhs, $jml_sks, '1', '1', 'A', '1', '-', '-', $status_pindah));
						$simpan = $this->s00_lib_api->get_api_jsob($api_url,'POST',$parameter);
					}
		...
		function cek_jalur_data_kat($kd_kat){
			$data = $this->api->get_api_json(
		...
		function cek_fn($kd = '198205112006042002'){
			...
			$data 		= $this->jml_mhs_ujian_semprop();
		...
		function get_current_bimbingan_semprop($periode = '1'){
			...
			foreach ($data as $bbg) {
				$mhs 	= $this->get_data_mhs($bbg['NIM']);
				$nilai_semprop = $this->get_status_semprop($bbg['NIM']);
				$waktu 	= $this->periode_semester(str_replace('-', '/', $bbg['TGL_MUNA']), $mhs[0]['KD_PRODI']);
				if($waktu){
					foreach ($waktu as $wkt) {
						if($ta == $wkt['KD_TA'] && $smt == $wkt['KD_SMT'] && $nilai_semprop){
							$current[] = $bbg;
						}
					}
				}				
		...
		function auto_membimbing_ta(){
			...
			foreach ($data as $ta) {
				...
				if($cek_nilai_mhs){
					...
					//CEK MEMBIMBING TA
					$status_pindah_a = $this->get_status_pindah(3);

					$jalur_data_a = 0;
					if($status_pindah_a == 0){
						$jalur_data_a = $this->cek_jalur_data_kat(3);
					}
					...
					//CEK KETUA SIDANG
					$status_pindah_b = $this->get_status_pindah($kd_kat);

					$jalur_data_b = 0;
					if($status_pindah_b == 0){
						$jalur_data_b = $this->cek_jalur_data_kat($kd_kat);
					}
					...
					if(!$cek_mhs_bimbingan_ta){
						...
						$api_url 	= URL_API_BKD.'bkd_beban_kerja/simpan_beban_kerja_sia_ta_maps';
						$parameter  = array('api_search'=>array($kd_jbk, $kd_dosen, $jenis_kegiatan, $bkt_penugasan, $sks_rule, $masa_penugasan, $bkt_dokumen, $sks_rule, $thn, $semester, $rekomendasi, $jml_jam, $capaian, $outcome, $file_penugasan, $file_capaian, $ta, $smt, $sts_pakai_a));
						$simpan = $this->s00_lib_api->get_api_jsob($api_url,'POST',$parameter);
						if($simpan){
							// echo 'simpan ke db bkd_beban_kerja dan BKD_REMUN_KINERJA sukses'; echo '<br>';
							// $api_url 	= URL_API_BKD.'bkd_beban_kerja/get_current_data_tersimpan';
							...
							//akan berubah menjadi kd_kat untuk membimbing
							$api_url 	= URL_API_BKD.'bkd_beban_kerja/simpan_data_pendidikan_ta';
							$parameter	= array('api_search' => array($getid, '3', $nm_keg, $jenjang, '-', '1', $jml_sks, '1', '1', 'A', '1', $prodi, $nim, $status_pindah_a));
							$simpan = $this->s00_lib_api->get_api_jsob($api_url,'POST',$parameter);
							if($simpan){
								//menimpan otomatis menjadi ketua sidang disini
								$jenis_kegiatan = $narasi2;
								$api_url 	= URL_API_BKD.'bkd_beban_kerja/simpan_beban_kerja_sia_ta_maps';
								$parameter  = array('api_search'=>array($kd_jbk, $kd_dosen, $jenis_kegiatan, $bkt_penugasan, $sks_rule2, $masa_penugasan, $bkt_dokumen, $sks_rule2, $thn, $semester, $rekomendasi, $jml_jam, $capaian, $outcome, $file_penugasan, $file_capaian, $ta, $smt, $sts_pakai_b));
								$simpan = $this->s00_lib_api->get_api_jsob($api_url,'POST',$parameter);
								...
								if($simpan && !$cek_mhs_sidang_ta){
									...
									//akan berubah menjadi kd_kat untuk membimbing
									$api_url 	= URL_API_BKD.'bkd_beban_kerja/simpan_data_pendidikan_ta';
									$parameter	= array('api_search' => array($getid, $kd_kat, $nm_keg, $jenjang, '-', '1', $jml_sks, '1', '1', 'A', '1', $prodi, $nim, $status_pindah_b));
									$simpan = $this->s00_lib_api->get_api_jsob($api_url,'POST',$parameter);
								}
		...
		function auto_fill_sks_penelitian(){
			$kategori = $this->input->post('kd_kat');
			$mhs = $this->input->post('jml_mhs');
			/*$kategori = 20;*/
			$nilai = $this->sksrule->_get_nilai_sks_penelitian($kategori, $mhs);
			echo $nilai;
		}
		function auto_fill_sks_pengabdian(){
			$kategori = $this->input->post('kd_kat');
			$nilai = $this->sksrule->_get_nilai_sks_pengabdian($kategori);
			echo $nilai;
		}
		function auto_fill_sks_penunjang(){
			$kategori = $this->input->post('kd_kat');
			$mhs = $this->input->post('jml_mhs');
			$nilai = $this->sksrule->_get_nilai_sks_penunjang($kategori, $mhs);
			echo $nilai;
		}
		...
		function auto_penelitian_for_remun(){
			$this->cek_nira_asesor_dosen_uin();
			$flash_remun = $this->session->flashdata('remunerasi');
			if($flash_remun){
				$this->session->set_flashdata('bebankerja', true);
		...
		function auto_penunjang_for_remun(){
			$this->auto_bimbingan_akademik();
			$flash_remun = $this->session->flashdata('remunerasi');
			if($flash_remun){
				$this->session->set_flashdata('bebankerja', true);
	...
	public function cek_nira_asesor_dosen_uin(){
			...
			// if($this->session->userdata('kd_dosen') == '197701032005011003'){
				...
				//do nothing
					...
					if($kd_bk){
						...
						if($status_pindah == 0){
							...
							//insert data asesor ke tabel BKD_BEBAN_KERJA dan BKD_REMUN_KINERJA 
						$api_url 	= URL_API_BKD.'bkd_beban_kerja/simpan_beban_kerja_sia_asesor_maps';
						$parameter  = array('api_search' => array($kd_jbk, $kd_dosen, $jenis_kegiatan, $bkt_penugasan, $sks_rule, $masa_penugasan, $bkt_dokumen, $sks_rule, $thn, $semester, $rekomendasi, $jml_jam, $capaian, $outcome, $file_penugasan, $file_capaian, $ta, $smt, $sts_pakai));
						$insert = $this->s00_lib_api->get_api_jsob($api_url,'POST',$parameter);
		...
		//api pengabdian belum dapat digunakan, karena harus disesuaikan dulu dengan sistem pengabdian yang digunakan oleh LPPM,
		//HAL INI BERKAITAN DENGAN SKS RULE termasuk jenis kategori apa yang akan di generate secara otomatis dari sistem
