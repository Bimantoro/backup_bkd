...
<div>
    ...
    <div id="option-menu1">
		<ul id="crumbs">
			...
			<li><a href="<?php echo base_url().'bkd/dosen/remun/data/'.$kode;?>" class="current"><?php echo ucwords($title);?></a>
			...
			<?php 
				if (empty($data_remun)){
					...
					<tr class ="<?php echo $bp->KD_BK;?>">
						<th><?php echo $no;?></th>
						<td class="JENIS_KEGIATAN"><a class="detail" title="Lihat detail beban kerja" href="<?php echo base_url().'bkd/dosen/remun/data/'.$kode.'/'.$bp->KD_BK;?>"><?php echo $bp->JENIS_KEGIATAN." Program Studi ".$bp->NM_PRODI.", Kelas ".$bp->KELAS.", ".$bp->JML_SKS." SKS, ".$bp->JML_MHS." Mahasiswa ".$bp->JML_DOSEN." Dosen";?></a></td>
						...
						<th><a href="<?php echo site_url().'bkd/dosen/remun/edit/'.$bp->KD_JBK.'/'.$bp->KD_BK;?>" class="btn btn-mini" <?php echo $tombol;?>>
						<i class="icon icon-edit"></i> Edit</a></th>
						<th><a class="btn btn-mini" onclick="return hapus('Anda yakin ingin menghapus data ini?')" href="<?php echo site_url().'bkd/dosen/remun/hapus_hybrid/'.$bp->KD_JBK.'/'.$bp->KD_BK;?>" <?php echo $tombol;?>><i class="icon icon-trash"></i> Hapus</a></th>
					</tr>
				<?php } ?>
				<!-- <tr class="total">
					<td></td>
	...
	<?php if($is_crud == 1){ ?>
		<p><a class="btn btn-uin btn-inverse btn-small" href="<?php echo base_url().'bkd/dosen/remun/tambah/'.$kode;?>">Tambah</a></p>
	<?php } ?>
			...
			<tr>
				...
				<table class="table table-condensed">
					...
					<tr>
						...
						<td><a href="<?php echo base_url().'bkd/dosen/remun/hapus_partner/'.$bp->KD_BK.'/PENELITIAN/'.$p->PARTNER;?>" class="btn btn-mini"><i class="icon-trash hapus-partner"></i></a></td>
					</tr>
					<?php } ?>
				</table>
				<?php }else { echo '-';}	?>			
