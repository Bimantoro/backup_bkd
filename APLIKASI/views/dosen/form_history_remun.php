		<br/>
		<?php 
			$cek_ta = $this->session->userdata('ta_remun');
			if($cek_ta){
				$temp_ta = $cek_ta;
			}else{
			...
			$cek_smt = $this->session->userdata('smt_remun');
			if($cek_smt){
				$temp_smt = $cek_smt;
			}else{
		...
		?>
		<?php $temp_smt = $this->session->userdata('smt');?>
		<?php 
		if($this->uri->segment(4) == 'kesimpulan'){ ?>
			<!-- kesimpulan -->
			<?php echo form_open('bkd/dosen/remun/kesimpulan');?>
		...
		<?php }else if ($this->uri->segment(4) == 'narasumber'){ ?>
			...
			<?php echo form_open('bkd/dosen/remun/narasumber');?>
				<input type="hidden" name="kode" value="<?php echo $this->uri->segment(4);?>">
				<table cellspacing="5" cellpadding="10" width="80%">
					<tr>
						<td>Tahun Akademik</td>
		...
		<?php } else{ ?>
			<!-- beban kerja -->
			<?php echo form_open('bkd/dosen/remun/history/'.$this->uri->segment(5).'/true');?>
